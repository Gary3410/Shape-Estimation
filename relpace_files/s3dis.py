...
class S3DISDataset(CustomDataset):
    ...
    CLASSES = ("banana", "bowl", "box", "can", "cup", "marker", "pear", "sugar")
    ...
    def get_filenames(self):
        ...
        for p in self.prefix:
            ...
            filenames = sorted(filenames * self.repeat, key=lambda x: int(os.path.basename(x).split("_")[3]))
            filenames_all.extend(filenames)
        # filenames是整体的路径
        # filenames_all = sorted(filenames_all * self.repeat) #key = lambda x: int(x[:-4])
        # filenames_all = sorted(filenames_all * self.repeat, key=lambda x: int(os.path.basename(x).split("_")[3]))
