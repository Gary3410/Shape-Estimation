...
class SoftGroup(nn.Module):
    ...
    def __init__(self,
        ...
        self.affinity_conv_1 = spconv.SparseSequential(
            norm_fn(32), nn.ReLU(),
            spconv.SparseConv3d(32, 32, kernel_size=2, stride=2, bias=False))

        self.affinity_conv_2 = spconv.SparseSequential(
            norm_fn(32), nn.ReLU(),
            spconv.SparseConv3d(32, 1, kernel_size=2, stride=2, bias=False))

        self.affinity_conv_3 = spconv.SparseSequential(
            norm_fn(1), nn.ReLU(),
            spconv.SparseConv3d(1, 256, kernel_size=2, stride=2, bias=False))
        ...
        if not semantic_only:
            ...
            # 这里就说明体素点云特征出来后, 需要经过spconv后, 继续跟普通的conv
    ...
    def forward(self, batch, return_loss=False, only_feature=False):
        if return_loss:
            return self.forward_train(**batch)
        elif only_feature:
            return self.forward_test_backbone(**batch)
        else:
    ...
    def forward_train(self, batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats,
        ...
        semantic_scores, pt_offsets, output_feats, output_sptensor = self.forward_backbone(input, v2p_map)
        #print(output_sptensor.spatial_shape)
        output_sptensor = self.affinity_conv_1(output_sptensor)
        output_sptensor = self.affinity_conv_2(output_sptensor)
        output_sptensor = self.affinity_conv_3(output_sptensor)
        ...
        loss_1, loss_2 = self.parse_losses(losses)
        return loss_1, loss_2, output_sptensor
    ...
    def forward_test(self, batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats,
        ...
        semantic_scores, pt_offsets, output_feats, feature_list = self.forward_backbone(
        ...
        return ret, feature_list

    @cuda_cast
    def forward_test_backbone(self, batch_idxs, voxel_coords, p2v_map, v2p_map, coords_float, feats,
                     semantic_labels, instance_labels, pt_offset_labels, spatial_shape, batch_size,
                     scan_ids, **kwargs):
        feats = torch.cat((feats, coords_float), 1)
        voxel_feats = voxelization(feats, p2v_map)
        ...
        semantic_scores, pt_offsets, output_feats, output_sptensor = self.forward_backbone(
            input, v2p_map, x4_split=self.test_cfg.x4_split)
        output_sptensor = self.affinity_conv_1(output_sptensor)
        output_sptensor = self.affinity_conv_2(output_sptensor)
        output_sptensor = self.affinity_conv_3(output_sptensor)
        return output_sptensor
    ...
    def forward_backbone(self, input, input_map, x4_split=False):
        ...
        return semantic_scores, pt_offsets, output_feats, output
