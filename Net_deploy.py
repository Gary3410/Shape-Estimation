...
def load_models(cat, cat1='pd', outf='1', train=False):
    ...
    if train:
        outf = '/home/lcl/fsnet_oper/%s/' % outf     # trained models
        temp =cat
        cat=cat1
        ...
        outf = '/home/lcl/fsnet_oper/yolov3_fsnet/trained_models'
        temp =cat
        cat='3mix_model'
        # cat = 'mix_all_ep200'
        Seg3d = '%s/Seg3D_last_obj%s.pth' % (outf, cat)
...
def load_models_my(cat, model_cat=None):
    classifier_seg3D = GCN3D_segR(class_num=2, vec_num=1, support_num=7, neighbor_num=10)
    classifier_ce = Point_center_res_cate()  ## translation estimation
    classifier_Rot_red = Rot_red(F=1296, k=6)  ## rotation red
    classifier_Rot_green = Rot_green(F=1296, k=6)  ### rotation green
    ...
    classifier_Rot_green.cuda()
    classifier_Rot_red.cuda()
    outf = '/home/potato/workplace/FS_Net-main/models/FS_Net_demo'
    temp = cat
    cat = 'mix_all'
    # cat = 'mix_all_ep200'
    Seg3d = '%s/Seg3D_last_obj%s.pth' % (outf, cat)
    ...
    model_sizes = np.loadtxt("/home/potato/workplace/obj_model/mean_scale_cls.txt")
    #model_size = model_sizes[cate_id0]
...
def FS_Net_Test_obj_size(points, pc, classifier_seg3D, classifier_ce, classifier_Rot_green, classifier_Rot_red,
                cat, model_size, cate_id0, num_cor=3, pts_rec=0):
    ...
    cate_id[0][0] = cate_id0 + 1
    one_hot = torch.zeros(points.shape[0], 16).scatter_(1, cate_id.cpu().long(), 1)
    #one_hot = torch.zeros(points.shape[0], 16).scatter_(1, cate_id.long(), 1)
    one_hot = one_hot.cuda()
    #print("pc", pc.shape)
    ...
    model = torch.from_numpy(model).unsqueeze(0).cuda()
    #model = model.cuda()
    #print("points", points_n.shape)
    #print(model.shape)
    points_n1 = torch.cat([points_n.to(model.device), model], dim=1)

    pred_seg, point_recon, feavecs = classifier_seg3D(points_n1, one_hot)
    pred_choice = pred_seg.data.max(2)[1]
    p = pred_choice

    ptsori = ptsori.cuda()
    #print(" ptsori", ptsori.shape)
    ptsori = torch.cat([ptsori, ptsori[:, np.random.choice(ptsori.shape[0], 500, replace=True), :]], dim=1)
