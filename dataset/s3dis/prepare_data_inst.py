...
def object_name_to_label(object_class):
    ...
    # 如果不存在则为clutter
    object_label = OBJECT_LABEL.get(object_class, OBJECT_LABEL["sugar"])
    return object_label
...
def read_s3dis_format(area_id: str,
    ...
    room_ver = pd.read_csv(raw_path, sep=" ", header=None, dtype=float).values
    xyz = np.ascontiguousarray(room_ver[:, 0:3], dtype="float32")
    #print(room_ver.shape)
    #print(room_ver.shape)
    #print(room_ver[:2, 3:6])
    #room_rgb = np.zeros_like(room_ver[:, 3:6])
    rgb = np.ascontiguousarray(room_ver[:, 3:6].astype(np.uint8), dtype="uint8")
    #rgb = np.ascontiguousarray(room_rgb, dtype="uint8")
    if not label_out:
        return xyz, rgb
    n_ver = len(room_ver)
    del room_ver
    ...
    for single_object in objects:
        ...
        # object_class需要修改一下
        object_class = '_'.join(object_name.split('_')[0:-1])
        # 根据object_class获得int型label
...
if __name__ == "__main__":
    ...
    for area_id in area_list:
        ...
        try:
            ...
            pass
        pbar = tqdm(room_name_list)
        for room_name in pbar:
