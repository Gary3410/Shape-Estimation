...
class Yolact(nn.Module):
    ...
    def __init__(self):
        ...
        self.prior_channels = 4761
        self.affinity_conv = nn.Conv2d(256, self.prior_channels, kernel_size=1)
        self.affinity_bn = nn.BatchNorm2d(self.prior_channels)

        #self.affinity_res_conv = nn.Conv2d(256, 256, kernel_size=1)
        #self.affinity_res_bn = nn.BatchNorm2d(256)
        #self.affinity_res_act = nn.ReLU(inplace=True)


        self.affinity_intra_conv = nn.Conv2d(256, 256, kernel_size=1)
        self.affinity_intra_bn = nn.BatchNorm2d(256)
        self.affinity_intra_act = nn.ReLU(inplace=True)

        self.affinity_inter_conv = nn.Conv2d(256, 256, kernel_size=1)
        self.affinity_inter_bn = nn.BatchNorm2d(256)
        self.affinity_inter_act = nn.ReLU(inplace=True)

        self.affinity_bottleneck_conv = nn.Conv2d(256 * 3, 256, kernel_size=1)
        self.affinity_bottleneck_bn = nn.BatchNorm2d(256)
        self.affinity_bottleneck_act = nn.ReLU(inplace=True)

        self.affinity_conv_1 = nn.Conv2d(256, 256, kernel_size=1)
        self.affinity_bn_1 = nn.BatchNorm2d(256)
        self.affinity_act_1 = nn.ReLU(inplace=True)

        #self.affinity_softmax = nn.Softmax(dim=0)
        ...
        for key in list(state_dict.keys()):
            ...
            if key.startswith('fpn.downsample_layers.'):
                if cfg.fpn is not None and int(key.split('.')[2]) >= cfg.fpn.num_downsample:
                    del state_dict[key]
        miss_keys, unexpect_keys = self.load_state_dict(state_dict, strict=False)
        if len(miss_keys) > 0:

            self.init_miss_keys()

    def init_miss_keys(self):
        for name, module in self.named_modules():
            if "affinity_res" in name:
                print(name)
                ...
                if is_conv_layer:
                    ...
                    if module.bias is not None:
                        print("初始换_bias", name)
                        if cfg.use_focal_loss and 'affinity' in name:
                            print("初始换_bias_focal", name)
    ...
    def freeze_layer(self, layer_list=[]):
    ...
    def forward(self, x, feature_list=None):
        ...
        if cfg.mask_type == mask_type.lincomb and cfg.eval_mask_branch:
            with timer.env('proto'):
                ...
                if feature_list != None:
                    point_feature = feature_list
                else:
                    point_feature = proto_x.clone()

                if point_feature.shape[-1] != proto_x.shape[-1] or point_feature.shape[-2] != proto_x.shape[-2]:
                    point_feature = F.interpolate(point_feature, size=(proto_x.shape[-2], proto_x.shape[-1]), mode='bilinear', align_corners=True)

                # resnet结构
                #point_feature = torch.add(point_feature, proto_x)
                #point_feature = self.affinity_res_conv(point_feature)
                #point_feature = self.affinity_res_bn(point_feature)
                #point_feature = self.affinity_res_act(point_feature)

                #point_feature = torch.add(point_feature, proto_x)

                point_feature = self.affinity_conv(point_feature)
                point_feature = self.affinity_bn(point_feature)
                point_feature = torch.sigmoid(point_feature)
                #point_feature = self.affinity_softmax(point_feature)


                batch_size, channels, height, width = proto_x.shape
                context_prior_map = point_feature.view(batch_size, height * width, -1)
                context_prior_map = context_prior_map.permute(0, 2, 1)
                inter_context_prior_map = 1 - context_prior_map

                # *10 or 100
                #point_feature = torch.mul(point_feature, 10)
                #inter_point_feature = torch.mul(inter_point_feature, 10)

                value = proto_x.view(batch_size, channels, -1)
                value = value.permute(0, 2, 1)

                intra_context = torch.bmm(context_prior_map, value)
                intra_context = intra_context.div(height * width)
                intra_context = intra_context.permute(0, 2, 1).contiguous()
                intra_context = intra_context.view(batch_size, channels, height, width)
                intra_context = self.affinity_intra_conv(intra_context)
                intra_context = self.affinity_intra_bn(intra_context)
                intra_context = self.affinity_intra_act(intra_context)

                inter_context = torch.bmm(inter_context_prior_map, value)
                inter_context = inter_context.div(height * width)
                inter_context = inter_context.permute(0, 2, 1).contiguous()
                inter_context = inter_context.view(batch_size, channels, height, width)

                inter_context = self.affinity_inter_conv(inter_context)
                inter_context = self.affinity_inter_bn(inter_context)
                inter_context = self.affinity_inter_act(inter_context)

                #inter_context = torch.mul(inter_context, 0)
                #intra_context = torch.mul(intra_context, 0)

                cp_outs = torch.cat([proto_x, intra_context, inter_context], dim=1)
                output = self.affinity_bottleneck_conv(cp_outs)
                output = self.affinity_bottleneck_bn(output)
                output = self.affinity_bottleneck_act(output)

                output = self.affinity_conv_1(output)
                output = self.affinity_bn_1(output)
                output = self.affinity_act_1(output)

                proto_out = self.proto_net(output)
        ...
        if self.training:
            ...
            return pred_outs, context_prior_map
        else:
            if cfg.use_mask_scoring:
