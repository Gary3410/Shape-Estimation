...
from yolov3_fsnet.utils.general import check_img_size, set_logging, increment_path
...
def detect(opt, data_path, classifier_seg3D, classifier_ce, classifier_Rot_green, classifier_Rot_red,
           model_size, cate_id0, cate,outf,epoch):
    ...
    # Set Dataloader
    dataset = LoadImages_fsnet(data_path, img_size=imgsz, stride=stride, train = True)
    ...
    for icc, data in enumerate(dataloader):
        ...
        # seg_gt = np.asarray(gt_seg[0])
        # y, x = np.where(seg_gt == 1)
        # dep_seg = np.zeros(depth.shape, dtype=np.float32)
        ...
        for i in range(1):  # detections per image
            ...
            if len(det):
                ...
                dep3d = dep3d[np.where(dep3d[:, 2] < 600)]
    ...
    eval_file = open('/home/lcl/fsnet_oper/%s/%s_eval_logs.txt' % (outf, cate), 'a')
    eval_file.write('epoch %d\n'%epoch)
    for ev in eva:
        print(ev)
        eval_file.write(ev + '\n')
    eval_file.close()
...
def det(cate,cat1,outf,epoch):
    ...
    # fold = '/home/lcl/fsnet_oper/yolov3_fsnet/data/%s_extend/' % cate
    fold = '/home/lcl/fsnet_oper/yolov3_fsnet/data/%s/' % cate
    ...
    classifier_seg3D, classifier_ce, classifier_Rot_green, classifier_Rot_red, model_size, cate_id0 = load_models(
        cate, cat1,outf,train=True)
    with torch.no_grad():
        detect(opt, fold, classifier_seg3D, classifier_ce, classifier_Rot_green,
               classifier_Rot_red, model_size, cate_id0, cate,outf,epoch)
