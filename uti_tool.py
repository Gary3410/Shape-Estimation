...
def defor_3D(pts,lab, R,T,pc ,scalex=(1,1),scalez=(1,1),scaley=(1,1),scale=(1,1), cate='',OR=None,bm=0.5):
    ...
    # show_mulit_mesh([pc])
    # show_pts(pts,'pts')
    # show_pts(ptsn,'pts_cen')

    ptsn[:, 0] = ptsn[:, 0] * ex
    ptsn[:, 1] = ptsn[:, 1] * ey
    ptsn[:, 2] = ptsn[:, 2] * ez
    # show_pts(ptsn, 'pts_cen_rescale')
    s=0
    ...
    # show_pts(ptsn, 'pts_cen_rescale_moveback')
    # print('inside func')
...
def var_2_norm(pc,ex,ey, ez,c=''):
    ...
    # print(lx,ly,lz,'       ',unitx,unity,unitz)
...
def show_3D_single(data, name):
    return 0
    # print(data.shape)
    # colormap = []
    # lab = np.asarray([[255, 0, 0]]) / 255.0
    ...
    # plt.savefig('/home/lcl/fsnet_test/yolov3_fsnet/data/' + '%s.png' % name, dpi=500, bbox_inches='tight', transparent=True)
    # plt.close()

def show_pts(data, name):
    # return 0
    print(data.shape)
    colormap = []
    lab = np.asarray([[255, 0, 0]]) / 255.0
