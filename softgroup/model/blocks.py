...
class UBlock(nn.Module):
    ...
    def forward(self, input, feature_list=[], idx=0):
        ...
        if len(self.nPlanes) > 1:
            ...
            idx = idx + 1
            output_decoder = self.u(output_decoder, feature_list, idx)
            ...
            #我只取一层feature
            # if idx > 4 and idx < 6:
               #feature_list.append(F.interpolate(torch.mean(output.dense(), dim=4),
               #                         size=(69,69),mode='bilinear', align_corners=True))
               #feature_list = F.interpolate(torch.mean(output.dense(), dim=4),
               #                       size=(69,69),mode='bilinear', align_corners=True)
